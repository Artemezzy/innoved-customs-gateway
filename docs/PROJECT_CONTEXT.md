...
### Поля публикации сертификата в подтверждённых позициях (2026-09-24)

На странице `/lk/confirmed-items` после файлового слота `final_document` («Сертификат / Письмо») размещаются:

- `certificate_url` — ссылка на запись в реестре сертификатов; менеджер и назначенный сертификационный центр могут установить, заменить или очистить ссылку; разрешены только абсолютные URL с `http://` или `https://`; клиент видит ссылку и может открыть её;
- `certificate_published` — отметка публикации сертификата; изменяет только менеджер, сертификационный центр и клиент видят текущее значение;
- `other_documents` — до пяти дополнительных файлов на подтверждённую позицию; менеджер и назначенный сертификационный центр загружают, клиент скачивает; менеджер удаляет любой файл, сертификационный центр — только файл, загруженный этим же пользователем сертификационного центра.

Допустимые расширения дополнительных документов: PDF, DOC, DOCX, XLS, XLSX, JPG, JPEG, PNG; максимальный размер одного файла — 20 МБ. Файлы хранятся в `uploads/confirmed/{confirmed_item_id}/other`, метаданные — в `lk_confirmed_item_other_documents`. Ограничение в пять файлов и права удаления обязательно проверяются на backend, UI только дублирует эти ограничения.
