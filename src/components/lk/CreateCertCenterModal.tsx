...
export function CreateCertCenterModal({ open, onOpenChange }: Props) {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: { name: '', contact_person: '', phone: '', email: '' },
  });

  const create = useMutation({
    mutationFn: (data: FormData) => lkApi.createCertCenter(data),
    onSuccess: (res) => {
      toast.success('Сертификационный центр создан');
    ...
    <Dialog open={open} onOpenChange={(v) => (v ? onOpenChange(v) : handleClose())}>
      <DialogContent>
        {!credentials ? (
          <>
            <DialogHeader>
              <DialogTitle>Создать сертификационный центр</DialogTitle>
            </DialogHeader>
            <form
          ...
          <>
            <DialogHeader>
              <DialogTitle>Учётные данные центра</DialogTitle>
            </DialogHeader>
            <div className="space-y-3 py-2">
              <p className="text-sm text-muted-foreground">
                Сохраните эти данные и передайте центру. После закрытия окна пароль не будет
                показан повторно.
              </p>
