 ...
 * AND prerendered H1/H2/H3 headings inside <div id="root"> for SEO crawlers.
 * This ensures search engine bots (especially Yandex) see correct meta
 * and heading structure without needing to execute JavaScript.
 */
...
function buildPrerenderedHeadings(page) {
  if (!page.h1) return '';
  const parts = [];
...
function processHtml(templateHtml, page) {
  ...
  // Inject prerendered headings into <div id="root">
  // These will be visible to crawlers in static HTML,
  // then replaced by React when it hydrates/mounts.
  const headingsHtml = buildPrerenderedHeadings(page);
  if (headingsHtml) {
    html = html.replace(
      '<div id="root"></div>',
      `<div id="root">${headingsHtml}</div>`
    );
  }
...
console.log(`✅ Prerendered meta tags + headings for ${created} pages`);
